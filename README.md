...
const productQuery = queryBuilder<Product>()
  .filter((op) => 
    op.and(
      ...
      op.isIn("id", ["p1", "p2"]),      // ✓ Valid list values
...
```typescript
import { eq, ne, lt, lte, gt, gte, between, isIn, isNotIn, isNull, isNotNull } from 'dyno-cql';
...
// List membership
isIn("investigation_type", ["CPT", "OTHER"]) // → investigation_type IN ('CPT', 'OTHER')
isNotIn("status", ["DELETED", "ARCHIVED"])     // → status NOT IN ('DELETED', 'ARCHIVED')
